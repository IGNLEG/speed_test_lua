...
elseif(args.best) then if test.download_server_list_json() then best_server, ping = test.find_best_server(test.read_server_list_json(), args.best) print(cjson.encode({best_server = best_server, ping = ping})) end
